    ...
    if event == "-FOLDER-":
        ...
        try:
            ...
            if (((CTX==CRX)and(filename!=None)and(modo=="ARDUINO"))or((CTX!=CRX)and(filename!=None)and(modo=="INTERNO"))):
                ...
                window["-ROUT3-"].update(client_header_resp[0])
                window["-ROUT4-"].update(client_header_resp[1])
                window["-ROUT5-"].update(client_header_resp[2])
                window["-ROUT6-"].update(client_header_resp[3])
                
                # Esperando os Dados e retornando a resposta
                client_data_resp = client_info.data_send_response()
                window["-ROUT7-"].update(client_data_resp[0])
                window["-ROUT8-"].update(client_data_resp[1])
                window["-ROUT9-"].update(client_data_resp[2]) 
